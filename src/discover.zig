...
fn discoverNodesWithNamespaces(allocator: std.mem.Allocator, node: *c.rcl_node_t) !void {
    ...
    var node_names_and_namespaces = c.rcl_get_zero_initialized_names_and_types();
    defer _ = c.rcl_names_and_types_fini(&node_names_and_namespaces);

    const alloc = c.rcutils_get_default_allocator();

    // Get node names with namespaces
    const ret = c.rcl_get_node_names_with_enclaves(node, alloc, &node_names_and_namespaces);

    if (ret != c.RCL_RET_OK) {
        std.debug.print("Failed to get node names with namespaces\n", .{});
        return;
    }

    std.debug.print("\n================================================================================\n", .{});
    std.debug.print("Discovered {} nodes:\n", .{node_names_and_namespaces.names.size});
    std.debug.print("================================================================================\n", .{});

    var i: usize = 0;
    while (i < node_names_and_namespaces.names.size) : (i += 1) {
        const name = node_names_and_namespaces.names.data[i];
        std.debug.print("  - {s}\n", .{name});
    }
    std.debug.print("\n", .{});
}
